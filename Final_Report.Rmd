---
title: "Analysis of the Effects of Diet on Progression of Dementia"
date: "2026-05-17"
output:
  pdf_document:
    toc: true
...
blood <- 
  read_csv("blood_data_update.csv")

diet <- 
  read_csv("diet_data_update.csv")

blood <- blood[,-1]
diet <- diet[,-1]

all <- 
  read_csv("Stats141XP_AABC_Release2_data.csv")
...
library(readxl)
all_dict <-
  read_excel("Stats141XP_AABC_study_data_dictionary.xlsx")
```
...
-   Notice that people are going from F1 –\> F2 instead of visiting : F1 –\> V2 –\> F2, so i want to see if they take about the same amount of time (yrs) to go from F1 –\> F2 as the people who went F1 –\> V2 –\> F2 – which would suggest people are going to the visit but we just aren't capturing their data.
...
ggplot(
  ...
  labs(
    title = "Blood Variables with Large Amount of Missing Data",
    x = "Variable",
    y = "Percent Missing"
  ) +
  expand_limits(y = 100) +
...
pct_vars_over_70 <-
  mean(miss_summary$pct_miss > 70) * 100
...
ggplot(
  ...
  labs(
    title = "Diet: Patient-Level Missing Data",
    x = "Number of Records per Patient",
...
ggplot(
  ...
  labs(
    title = "Blood: Patient-Level Missing Data",
    x = "Number of Records per Patient",
    y = "Total Missing Values"
  ) +
  theme_minimal() +
...
ggplot(
  ...
  labs(
    title = "Distribution of Maximum Visit Reached",
    x = "Maximum Visit",
...
blood_summary_reloaded <-
  ...
  summarise(
    ...
    ureanitrogen =
      mean(ureanitrogen, na.rm = TRUE),
    
    creatinine =
      mean(creatinine, na.rm = TRUE),
    
    testosterone =
      mean(testosterone, na.rm = TRUE),
    
    hba1c =
      mean(hba1c, na.rm = TRUE),
    
   estradiol =
     mean(estradiol, na.rm = TRUE),
...
diet_summary_reloaded <-
  ...
  summarise(
    ...
    asa24_numfoods =
      mean(asa24_numfoods, na.rm = TRUE),
    
    asa24_mois =
      mean(asa24_mois, na.rm = TRUE),
    
    asa24_ff =
      mean(asa24_ff, na.rm = TRUE),
    
    asa24_f_juice =
      mean(asa24_f_juice, na.rm = TRUE),
...
nrow(retention_master_reloaded)
nrow(model_data)
```
...
model_comparison <- data.frame(
  ...
  Fixed_Effects = c(
    "Age + Sex + HbA1c + Fiber Intake",
    "Age + Sex + HbA1c + Insulin + Fiber Intake"
  ),
  Parameters = c(7, 8),
  AIC = c(2028.3, 2030.3),
  BIC = c(2061.1, 2067.7),
  LogLik = c(-1007.2, -1007.2),
  Chisq = c(NA, 0.0066),
  Df = c(NA, 1),
  p_value = c(NA, 0.9353)
)
...
model_comparison |>
  kable(
    caption = "Comparison of Mixed-Effects Models Predicting Memory",
    digits = 4,
    align = "l"
  ) |>
  kable_styling(
...
model_comparison |>
  gt() |>
  tab_header(
    title = "Mixed-Effects Model Comparison"
  ) |>
  fmt_number(
    columns = c(AIC, BIC, LogLik, Chisq, p_value),
    decimals = 4
  ) |>
  tab_style(
    style = cell_fill(color = "#e3f5fa"),
